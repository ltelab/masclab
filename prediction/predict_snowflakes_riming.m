function predict_snowflakes_riming(dir_data,t_str_start,t_str_stop,classifier,save_results,blurry_threshold)
    ...
    fprintf('Classifying snowflakes riming degree in : %s...\n',dir_data);
    ...
    [X,Xlab,Xname,~] = load_processed_data(dir_data,t_str_start,t_str_stop,classifier.feat_vec);
    ...
    if save_results
        ...
        for i=1:numel(Xname)
            ...
            if roi.xhi > blurry_threshold
                roi.riming_ID = pred(i);
                roi.riming_name = prediction_scheme{pred(i)};
                roi.riming_probs = scores(i,:);
                %roi.riming_idx = compute_riming_idx(1*roi.riming_probs(1) + 2*roi.riming_probs(2) + 3.5*roi.riming_probs(3) + 4.5*roi.riming_probs(4) + 5*roi.riming_probs(5));
                N_fine = N_fine + 1;
            else
                roi.riming_ID = -9;
                roi.riming_name = 'blurry';
                roi.riming_probs = scores(i,:);
                %roi.riming_idx = compute_riming_idx(1*roi.riming_probs(1) + 2*roi.riming_probs(2) + 3.5*roi.riming_probs(3) + 4.5*roi.riming_probs(4) + 5*roi.riming_probs(5));
                N_blurry = N_blurry + 1;
            end
