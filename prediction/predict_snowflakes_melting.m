function predict_snowflakes_melting(dir_data,t_str_start,t_str_stop,classifier,save_results,blurry_threshold)
    ...
    fprintf('Classifying snowflakes between dry and melting in : %s...\n',dir_data);
    ...
    [X,~,Xname,~] = load_processed_data(dir_data,t_str_start,t_str_stop,classifier.feat_vec);
    ...
    if save_results
        ...
        for i=1:numel(Xname)
            ...
            if roi.xhi > blurry_threshold
                roi.melting_ID = pred(i);
                if ~isnan(pred(i))
                    roi.melting_name = prediction_scheme{pred(i)+1};
                else
                    roi.melting_name = 'NaN... trololo';
                end
                roi.melting_probs = scores(i,:);
                N_fine = N_fine + 1;
            else
                roi.melting_ID = -9;
                roi.melting_name = 'blurry';
                roi.melting_probs = scores(i,:);
                N_blurry = N_blurry + 1;
            end
