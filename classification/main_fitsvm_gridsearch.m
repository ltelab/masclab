...
X = [X score(:,1:20)];% score(:,1:10) score2(:,1:10)];    score(:,1:20)
...
boxconstraint_vec = 464;%logspace(2,4,10); %1000;%[1e-4 1e-3 1e-2 1e-1 1 1e1 1e2 1e3 1e4 1e5];
kernelscale_vec = 28;%logspace(1,3,10);%100;%[1e-4 1e-3 1e-2 1e-1 1 1e1 1e2 1e3 1e4 1e5];
...
for i = 1:N_it
    ...
    for k = 1:K
        ...
        for j=1:N_classes
            ...
            SVMModel = fitcsvm(XTr,y_tmp,'BoxConstraint',boxconstraint_vec(m),'KernelFunction','linear','KernelScale',kernelscale_vec(n)); %'auto'
            % update the model to fit posterior probabilities
            model{j} = SVMModel;%fitPosterior(SVMModel);
...
% keep mean global scores in matrix MxN
grid_search_BER_Te(m,n) = mean(BER_Te(:));
grid_search_BER_Tr(m,n) = mean(BER_Tr(:));
grid_search_kappa_Te(m,n) = mean(kappa_Te(:));
grid_search_kappa_Tr(m,n) = mean(kappa_Tr(:));
